...
//  黑白棋   
//  
//  Created by acdzh on 2017/12/1.  
//  Copyright © 2017 acdzh. All rights reserved.
//                                              
...
int main()
{
	...
	my_color = -2 * my_color + 1;//将平台的表示转化为自己的表示;
...
int count( int color)
{
	...
	for (int i  = 0; i <= 15; i++)
	{
		for (int j = 0; j <= 15; j++)
		{
			...
			if (number == 1 && chess[i][j] == 1){ number += 1; }
			else if (number == -1 && chess[i][j] == -1) { number += 1; }
			else { continue; }
		}
